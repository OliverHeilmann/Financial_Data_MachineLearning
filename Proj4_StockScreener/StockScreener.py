...
def graphData(stock,MA1,MA2):
    ...
    try:
        ...
        try:
            netIncome = URL.urlopen('http://www.quandl.com/api/v1/datasets/OFDP/DMDRN_'+stock.upper()+'_NET_INC.csv?&'+endLink).read()
            revenue = URL.urlopen('http://www.quandl.com/api/v1/datasets/OFDP/DMDRN_'+stock.upper()+'_REV_LAST.csv?&'+endLink).read()
            ROC = URL.urlopen('http://www.quandl.com/api/v1/datasets/OFDP/DMDRN_'+stock.upper()+'_ROC.csv?&'+endLink).read()
...
def screener(stock):
    ...
    try:
        #print('doing',stock)
        sourceCode = URL.urlopen('http://finance.yahoo.com/q/ks?s='+stock).read().decode('utf-8')
        pdb.set_trace()
        
        pbrat = 'Price/Book</span><!-- react-text: 160 --> <!-- /react-text --><!-- react-text: 161 -->(mrq)<!-- /react-text --><sup aria-label="" data-reactid="162"></sup><div class="W(3px) Pos(a) Start(100%) T(0) H(100%) Bg($pfColumnFakeShadowGradient) Pe(n) Pend(5px)" data-reactid="163"></div></td><td class="Ta(c) Pstart(10px) Miw(60px) Miw(80px)--pnclg Bgc($lv1BgColor) fi-row:h_Bgc($hoverBgColor)" data-reactid="164">'
